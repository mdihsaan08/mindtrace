...
function getTrend(scores:number[]):"improving"|"declining"|"stable" {
...
export default function PatternsPage() {
  ...
  useEffect(()=>{
    async function load() {
      try {
        ...
        setEntries((d.entries??[]).filter((e:Entry)=>typeof e.moodScore==="number"));
  ...
  const statCards=[
    {label:"Average mood", value:avg, sub:"out of 10"},
    {label:"Trend", value:trendConf.label, sub:"recent vs. earlier", color:trendConf.color},
    {label:"Best score", value:`${best}/10`, sub:"your peak"},
    {label:"Lowest score", value:`${worst}/10`, sub:"your low point"},
  ];
  ...
  return (
    <div className="fade-up" style={{display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {statCards.map(s=>(
          <div key={s.label} style={{background:"white",borderRadius:12,border:`1px solid ${P.blush}44`,padding:"16px 18px",boxShadow:`0 1px 6px ${P.plum}06`}}>
            <p style={{fontSize:10,fontWeight:700,color:P.blush,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:6}}>{s.label}</p>
            <p style={{fontFamily:"Georgia,serif",fontSize:22,fontWeight:500,color:s.color??P.plum,lineHeight:1,marginBottom:4}}>{s.value}</p>
      ...
      {/* Chart */}
      <div style={{background:"white",borderRadius:16,border:`1px solid ${P.blush}44`,padding:20,boxShadow:`0 1px 6px ${P.plum}06`}}>
            ...
            {topKw.map(([kw,count])=>(
              <div key={kw} style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <div style={{flex:1,height:6,borderRadius:999,background:`${P.blush}33`,overflow:"hidden"}}>
                  <div style={{height:"100%",borderRadius:999,width:`${(count/topKw[0][1])*100}%`,background:`linear-gradient(90deg,${P.plum},${P.mauve})`,transition:"width 0.5s"}}/>
                </div>
      ...
      <div style={{background:`linear-gradient(135deg,#F4F1F6,#FDF8F8)`,borderRadius:16,border:`1px solid ${P.blush}44`,padding:20}}>
        ...
        <p style={{fontFamily:"Georgia,serif",fontStyle:"italic",color:P.plum,fontSize:14,lineHeight:1.8}}>
          Across your {entries.length} entries, your average mood sits at {avg}/10 and is{" "}
          {trend==="improving"?"on an upward trajectory — something positive seems to be building":trend==="declining"?"trending downward — it may be worth reflecting on what's been weighing on you":"holding steady"}.
