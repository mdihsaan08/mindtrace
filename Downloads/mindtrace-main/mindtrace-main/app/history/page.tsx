...
export default function HistoryPage() {
  ...
  async function fetch_(p:number) {
    setLoading(true);
    try {
      const res = await fetch(`/api/entries?page=${p}&limit=10`);
      const d = await res.json();
      setEntries(d.entries??[]); setTotalPages(d.pagination?.pages??1); setTotal(d.pagination?.total??0);
  ...
  return (
    <div className="fade-up" style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      {loading ? (
        ...
        <div style={{ background:"white", borderRadius:16, border:`1.5px dashed ${P.blush}`, padding:"60px 24px", textAlign:"center", boxShadow:`0 2px 12px ${P.plum}06` }}>
          ...
          {totalPages>1&&(
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, paddingTop:8 }}>
              <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1}
                style={{ padding:"7px 16px", borderRadius:9, border:`1px solid ${P.blush}`, background:"white", color:P.mauve, fontSize:12, cursor:"pointer", opacity:page===1?0.4:1 }}>← Previous</button>
              <span style={{ fontSize:13, color:P.mauve }}>Page {page} of {totalPages}</span>
              <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
                style={{ padding:"7px 16px", borderRadius:9, border:`1px solid ${P.blush}`, background:"white", color:P.mauve, fontSize:12, cursor:"pointer", opacity:page===totalPages?0.4:1 }}>Next →</button>
            </div>
          )}
        </div>
      )}
