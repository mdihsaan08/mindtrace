...
export default function DashboardPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.code==="RATE_LIMIT"?"⏱ Rate limit reached. Wait 30 seconds.":data.code==="TIMEOUT"?"⏳ Request timed out. Try again.":data.error??"Something went wrong.");
        return;
      }
    ...
    <div className="fade-up" style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
        <div>
          ...
          <h1 style={{ fontFamily:"Georgia,serif", fontSize:26, fontWeight:500, color:P.plum, letterSpacing:"-0.3px" }}>How are you doing?</h1>
        </div>
        {result && (
          <button onClick={()=>{setResult(null);setShowChat(false);}} style={{ fontSize:12, color:P.mauve, background:"none", border:`1px solid ${P.blush}`, borderRadius:8, padding:"6px 12px", cursor:"pointer", marginTop:4 }}>
            + New entry
          </button>
        )}
      </div>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:18 }}>
          ...
          <div>
            ...
            <textarea value={content} onChange={e=>setContent(e.target.value)} maxLength={5000} rows={10}
              ...
              style={{ width:"100%", padding:"16px 18px", borderRadius:14, border:`1px solid ${P.blush}77`, background:"white",
                fontFamily:"Georgia,serif", fontStyle:"italic", fontSize:15, color:P.plum, lineHeight:1.8,
                resize:"none", outline:"none", boxShadow:`0 2px 12px ${P.plum}06`,
                transition:"border-color 0.15s",
              }}
              onFocus={e=>e.target.style.borderColor=P.mauve}
              onBlur={e=>e.target.style.borderColor=`${P.blush}77`}/>
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:6, padding:"0 2px" }}>
              <span style={{ fontSize:11, color:P.blush }}>
                {!isReady&&charCount>0 ? `${20-charCount} more characters to unlock` : isReady ? `${wordCount} words` : ""}
              </span>
              <span style={{ fontSize:11, color:P.blush }}>{charCount}/5000</span>
            </div>
            {/* Progress bar */}
            {charCount>0&&!isReady&&(
              <div style={{ height:2, background:`${P.blush}33`, borderRadius:999, marginTop:6, overflow:"hidden" }}>
                <div style={{ height:"100%", borderRadius:999, width:`${(charCount/20)*100}%`, background:`linear-gradient(90deg,${P.blush},${P.mauve})`, transition:"width 0.3s" }}/>
              </div>
            )}
          </div>
          ...
          {/* Error */}
          {error && (
            <div style={{ fontSize:13, color:"#c0392b", background:"#fdf0ef", border:"1px solid #f5c6c2", borderRadius:10, padding:"10px 14px", display:"flex", gap:8 }}>
          ...
          <button type="submit" disabled={loading||!isReady}
            style={{ padding:"13px", borderRadius:12, background: isReady ? P.plum : `${P.plum}60`, color:P.cream, fontSize:14, fontWeight:500, border:"none", cursor: isReady?"pointer":"not-allowed",
              boxShadow: isReady ? `0 4px 16px ${P.plum}35` : "none", transition:"all 0.2s" }}>
            {loading ? (
              <span style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
                <span style={{ display:"inline-flex", gap:4 }}>
                  {[0,1,2].map(i=><span key={i} className="pulse-dot" style={{ width:6, height:6, borderRadius:"50%", background:P.cream, display:"inline-block", animationDelay:`${i*0.2}s` }}/>)}
                </span>
