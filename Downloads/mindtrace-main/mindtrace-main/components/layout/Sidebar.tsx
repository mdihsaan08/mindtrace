  ...
  return (
    <aside style={{ width:220, minHeight:"100vh", background:P.plum, display:"flex", flexDirection:"column", padding:"24px 12px", flexShrink:0 }}>
          ...
          return (
            <Link key={link.href} href={link.href} style={{ textDecoration:"none" }}>
              <div style={{
                ...
                background: active ? "rgba(253,220,176,0.12)" : "transparent",
                border: active ? `1px solid rgba(253,220,176,0.15)` : "1px solid transparent",
                transition:"all 0.15s",
              }}>
                <div style={{ width:30, height:30, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0,
                  background: active ? P.cream : "rgba(206,174,176,0.15)",
                  color: active ? P.plum : P.blush, fontSize:12,
                }}>
                  {link.icon}
                </div>
                <div>
      ...
      <button onClick={() => signOut({ callbackUrl:"/" })}
        style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", borderRadius:10, background:"transparent", border:"none", cursor:"pointer", width:"100%", transition:"background 0.15s" }}
        onMouseEnter={e => (e.currentTarget.style.background = "rgba(206,174,176,0.1)")}
