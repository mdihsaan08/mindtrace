...
export function ChatPanel({ entryContent, analysis, onClose }: { entryContent:string; analysis:AnalysisResult; onClose:()=>void }) {
  const [messages, setMessages] = useState<Message[]>([{ role:"assistant", content:`I've read your entry and noticed you're feeling ${analysis.moodLabel}. ${analysis.insight} What would you like to explore further?` }]);
  ...
  return (
    <div className="fade-up" style={{ background:"white", borderRadius:16, border:`1px solid ${P.blush}55`, display:"flex", flexDirection:"column", height:520, boxShadow:`0 2px 16px ${P.plum}08` }}>
      {/* Header */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 20px", borderBottom:`1px solid ${P.blush}33` }}>
        ...
        <button onClick={onClose} style={{ width:28, height:28, borderRadius:7, background:`${P.blush}22`, border:"none", color:P.mauve, cursor:"pointer", fontSize:12, display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
      </div>
  ...
  background: m.role === "user" ? P.plum : "linear-gradient(135deg,#F4F1F6,#FDF8F8)",
  color: m.role === "user" ? P.cream : P.plum,
  fontSize: 13,
  lineHeight: 1.6,
  fontFamily: m.role === "assistant" ? "Georgia,serif" : "inherit",
  fontStyle: m.role === "assistant" ? "italic" : "normal",
  border: m.role === "assistant" ? `1px solid ${P.blush}33` : "none",
}}>{m.content}</div>
          </div>
        ))}
        {loading && (
      ...
      <div style={{ padding:"12px 16px", borderTop:`1px solid ${P.blush}33` }}>
        <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
          ...
          <button onClick={send} disabled={loading||!input.trim()}
            style={{ width:38, height:38, borderRadius:10, background:P.plum, border:"none", color:P.cream, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0, boxShadow:`0 2px 8px ${P.plum}30`, opacity:loading||!input.trim()?0.4:1 }}>↑</button>
        </div>
