...
export function AnalysisCard({ analysis, onStartChat }: { analysis:AnalysisResult; onStartChat:()=>void }) {
  const ss = scoreStyle(analysis.moodScore);
  return (
    <div className="fade-up" style={{ background:"white", borderRadius:16, border:`1px solid ${P.blush}55`, padding:24, boxShadow:`0 2px 16px ${P.plum}08` }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:16, paddingBottom:18, borderBottom:`1px solid ${P.blush}33`, marginBottom:18 }}>
        <div style={{ width:60, height:60, borderRadius:14, background:ss.bg, border:`1.5px solid ${ss.border}`, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
          <span style={{ fontFamily:"Georgia,serif", fontSize:22, fontWeight:600, color:ss.text, lineHeight:1 }}>{analysis.moodScore}</span>
          <span style={{ fontSize:10, color:ss.text, opacity:0.6 }}>/10</span>
        </div>
        <div style={{ flex:1 }}>
        ...
        <span style={{ fontSize:11, fontWeight:600, padding:"5px 12px", borderRadius:999, background:ss.bg, border:`1px solid ${ss.border}`, color:ss.text }}>{ss.label}</span>
      </div>
      ...
      <button onClick={onStartChat}
        style={{ width:"100%", padding:"11px", borderRadius:10, border:`1.5px dashed ${P.blush}`, background:"transparent", color:P.mauve, fontSize:13, fontWeight:500, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, transition:"all 0.15s" }}
        onMouseEnter={e=>{e.currentTarget.style.background=`${P.blush}15`;e.currentTarget.style.color=P.plum}}
        onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color=P.mauve}}>
